...
export async function GET(
  ...
  try {
    ...
    // Ensure sessionId has 'thread_' prefix for the agent service
    const sessionIdForAgent = sessionId.startsWith('thread_') ? sessionId : `thread_${sessionId}`;

    // Forward request to Agent Service to retrieve chat history
    const response = await fetch(`${AGENT_SERVICE_URL}/chat/history/${sessionIdForAgent}`, {
      method: 'GET',
      headers: {
