...
export async function POST(request: NextRequest) {
  try {
    ...
    // If sessionId is provided, ensure it has 'thread_' prefix for the agent service
    let sessionIdForAgent: string | null = null;
    if (sessionId) {
      // Only add prefix if it doesn't already have it
      sessionIdForAgent = sessionId.startsWith('thread_') ? sessionId : `thread_${sessionId}`;
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        sessionId: data.session_id, // Return full session_id with 'thread_' prefix
      }),
