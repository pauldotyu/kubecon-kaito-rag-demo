...
const getUrlSessionId = (sessionId: string) => 
  sessionId.replace(THREAD_PREFIX, '');
...
export function useChat(initialSessionId?: string) {
  ...
  const [sessionId, setSessionId] = useState<string | null>(
    initialSessionId ? `${THREAD_PREFIX}${initialSessionId}` : null
  );
  ...
  useEffect(() => {
    ...
    const fetchChatHistory = async () => {
      try {
        ...
        const urlSessionId = getUrlSessionId(sessionId);
        console.log('[useChat] Fetching chat history for session:', urlSessionId);
        const response = await fetch(`/api/chat/history/${urlSessionId}`);
        ...
        setState(prev => {
          ...
          if (data.sessionId && data.sessionId !== sessionId) {
            ...
            setTimeout(() => {
              ...
              router.push(`/${getUrlSessionId(data.sessionId)}`);
            }, 0);
          }
